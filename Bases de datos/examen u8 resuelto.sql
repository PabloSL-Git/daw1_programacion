...
begin
	...
	if old.precioartpromo <> new.precioartpromo 
		and new.precioartpromo >= (select precioventa from articulos where refart = new.refart) then
			...
			signal sqlstate  '01234' set message_text  = 'el precio no se ha modificado';
	end if;
end $$
delimiter ;
...
Para comenzar nos piden que lo dejemos preparado para que, desde hoy MIERCOLES,  
...
every 2 quarter
-- every 6 month
	-- starts curdate() + interval 4
    -- starts curdate() + interval '5 11' day_hour
    starts curdate() + interval 4 day + interval 11 hour
    -- starts '2025-05-11 00:00'
    -- ends curdate() + interval '5 0' day_hour + interval 1 year
    ends '2025-05-11 00:00' + interval 1 year
